...
export class QueryBuilder {
	...
	buildUpdateQuery<TEntity>(options: IUpdateOptions<TEntity>): { query: string; values: any[] } {
		...
		const { clause: whereClause, values: whereValues } = this.buildClauseWhere(where, dataKeys.length + 1)
	...
	private buildClauseOrderBy<TEntity>(orderBy?: TOrderBy<TEntity>) {
		if (!orderBy) {
			return ""
		}
		...
		return `ORDER BY ${keys.map((key) => `${key} ${orderBy[key as keyof TOrderBy<TEntity>]?.toUpperCase()}`).join(", ")}`
	}

	private isIWhereOperator<TEntity>(
		value: TEntityValue<TEntity> | TEntityValue<TEntity>[]
	): value is TWhereOperator<TEntity> {
	...
	private buildWhereCondition<TEntity>(
		key: keyof TWhere<TEntity>,
		value: TEntityValue<TEntity>,
		paramIndex: { value: number }
	): string {
